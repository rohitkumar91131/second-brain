...
export default function GoalDetailScreen({ route, navigation }) {
  const existing = route.params?.goal ?? null
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const payload = { title: title.trim(), status, progress: Math.min(100, Math.max(0, parseInt(progress) || 0)), dueDate, description }
      if (isNew) await goalsAPI.create(payload)
      else await goalsAPI.update(existing.id, payload)
      navigation.goBack()
    } catch (e) {
  ...
  const del = () => {
    Alert.alert('Delete Goal', 'Are you sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => { try { await goalsAPI.delete(existing.id); navigation.goBack() } catch (e) { Alert.alert('Error', e.message) } } },
    ])
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.navbar}>
        ...
        <Text style={styles.navTitle}>{isNew ? 'New Goal' : 'Edit Goal'}</Text>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <TextInput style={styles.titleInput} placeholder="Goal title..." placeholderTextColor={COLORS.textMuted} value={title} onChangeText={setTitle} multiline />
        ...
        <Section label="Progress (0-100)"><TextInput style={styles.input} placeholder="0" placeholderTextColor={COLORS.textMuted} value={progress} onChangeText={setProgress} keyboardType="numeric" /></Section>
        <Section label="Target Date (yyyy-MM-dd)"><TextInput style={styles.input} placeholder="e.g. 2025-12-31" placeholderTextColor={COLORS.textMuted} value={dueDate} onChangeText={setDueDate} /></Section>
        <Section label="Description"><TextInput style={[styles.input, { minHeight: 100 }]} placeholder="Describe your goal..." placeholderTextColor={COLORS.textMuted} value={description} onChangeText={setDescription} multiline textAlignVertical="top" /></Section>
      </ScrollView>
    </View>
  )
}
...
function Section({ label, children }) {
  return <View style={{ gap: SPACING.xs, marginBottom: SPACING.md }}><Text style={{ color: COLORS.textMuted, fontSize: FONT_SIZE.xs, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</Text>{children}</View>
}

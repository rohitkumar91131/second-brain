...
export default function ViewSwitcher({ activeView, onViewChange, tabName = 'Tasks' }) {
    ...
    return (
        <div className="relative">
            <div className="flex items-center gap-0.5 border border-notion-border rounded-md p-0.5 bg-notion-sidebar">
            ...
            {menu && (
                <div
                    className="fixed z-[999] bg-notion-bg border border-notion-border rounded-lg shadow-xl py-1 min-w-[200px] animate-in fade-in zoom-in duration-100"
                    style={{ top: menu.y, left: menu.x }}
                >
                    <button
                        onClick={handleSetDefault}
                        className="w-full text-left px-3 py-2 text-xs font-medium text-notion-text hover:bg-[#f1f1ef] transition-colors flex items-center gap-2"
                    >
